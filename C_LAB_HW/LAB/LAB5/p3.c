...
int get_match_result(int target, int guessed){
	...
	for (int j = 0 ; j <3; j++){
		for (int k = 0; k <3; k++){
			if(target_digits[j] != guessed_digits[j] && target_digits[j] == guessed_digits[k]) n_ball++; 
		}
		if (target_digits[j] == guessed_digits[j]) n_strike++;
	}
